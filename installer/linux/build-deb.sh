...
rm -rf "$PKG"
mkdir -p "$PKG/DEBIAN" "$PKG/opt/localvoice" "$PKG/usr/bin" "$PKG/usr/share/applications" "$PKG/usr/share/icons/hicolor/256x256/apps"
...
Recommends: wtype
Description: Local offline speech-to-text and translation for Windows and Linux.
EOF
...
cat > "$PKG/usr/share/applications/localvoice.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=LocalVoice
Exec=localvoice
...
dpkg-deb --build "$PKG" "$ROOT/release/linux/LocalVoice-${VERSION}-amd64.deb"
