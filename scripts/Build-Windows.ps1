...
if (-not $SkipDependencyAudit) {
    # Argos Translate 1.11.0 currently pins stanza==1.10.1.
    # Ignore only its known transitive advisory until Argos releases a compatible update.
    & $Python -m pip_audit `
        -r requirements.txt `
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "The frozen Windows application failed its package smoke test."
    }
} finally {
...
Compress-Archive `
    -Path "$Release\LocalVoice\*" `
    -DestinationPath "$Release\LocalVoice-Windows-x64-Portable.zip" `
    -Force
...
if ($SignTool -and $Thumbprint) {
    & $SignTool sign `
        ...
        /td SHA256 `
        (Join-Path $Release "LocalVoice-Setup-Windows-x64.exe")
} else {
    "UNSIGNED BUILD - configure SIGNTOOL_PATH and WINDOWS_CERT_THUMBPRINT for public distribution." |
        Set-Content `
            (Join-Path $Release "SIGNING_STATUS.txt") `
            -Encoding UTF8
}

& $Python -m pip freeze |
    Set-Content `
        (Join-Path $Release "PYTHON-DEPENDENCIES.txt") `
        -Encoding UTF8
